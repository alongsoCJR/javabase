package focus.javabase.com.leetcode;

import focus.javabase.com.algorithm.sort.ArrayUtils;
import focus.javabase.com.baseTest.AbstractTest;
import org.junit.Assert;
import org.junit.Before;
import org.junit.Test;

import java.util.Arrays;


//There is an integer array nums sorted in ascending order (with distinct values
//).
//
// Prior to being passed to your function, nums is possibly rotated at an unknow
//n pivot index k (1 <= k < nums.length) such that the resulting array is [nums[k]
//, nums[k+1], ..., nums[n-1], nums[0], nums[1], ..., nums[k-1]] (0-indexed). For
//example, [0,1,2,4,5,6,7] might be rotated at pivot index 3 and become [4,5,6,7,0
//,1,2].
//
// Given the array nums after the possible rotation and an integer target, retur
//n the index of target if it is in nums, or -1 if it is not in nums.
//
// You must write an algorithm with O(log n) runtime complexity.
//
//
// Example 1:
// Input: nums = [4,5,6,7,0,1,2], target = 0
//Output: 4
// Example 2:
// Input: nums = [4,5,6,7,0,1,2], target = 3
//Output: -1
// Example 3:
// Input: nums = [1], target = 0
//Output: -1
//
//
// Constraints:
//
//
// 1 <= nums.length <= 5000
// -104 <= nums[i] <= 104
// All values of nums are unique.
// nums is an ascending array that is possibly rotated.
// -104 <= target <= 104
//
// Related Topics 数组 二分查找
// 👍 2320 👎 0

public class RotatedSortedT033Test extends AbstractTest {

    //    @Before
    public void setUp0() throws Exception {
        inputNum = 2;
        inputNums = new int[]{4, 5, 6, 7, 0, 1, 2};
        exceptedNum = 6;
    }


    @Before
    public void setUp2() throws Exception {
        inputNum = 8;
        inputNums = new int[]{4, 5, 6, 7, 8, 1, 2, 3};
        exceptedNum = 4;
    }

    @Test
    public void search() {
        Assert.assertEquals(exceptedNum, RotatedSortedT033.search(inputNums, inputNum));
    }

    /**
     * @Author Jianrong.Chen
     * @Description info
     * Wrong Answer:
     * input:[4,5,6,7,8,1,2,3]
     * 8
     * Output:-1·
     * @Date 2022-09-15 20:19
     * @Param []
     **/

    //    @Before
    public void setUp1() throws Exception {
        inputNum = 7;
        inputNums = new int[]{0, 1, 2, 4, 5, 6, 7};
        exceptedNum = 6;
    }

    @Test
    public void binarySearch() {
        ArrayUtils.maoPaoSort(inputNums);
        Assert.assertEquals(exceptedNum, RotatedSortedT033.binarySearch(inputNums, inputNum));
    }


}